...
if not db_exists:
    ...
    try:
        cursor.execute("SELECT source FROM headlines LIMIT 1")
    except sqlite3.OperationalError:
        # Drop and recreate the table if schema mismatch
...
def classify_headline(headline):
    ...
    if any(word in headline.lower() for word in tragedy_keywords):
        category = "World News"
    return category

def save_to_db(source, headline, category):
    cursor.execute(
        "INSERT INTO headlines (source, headline, category, scraped_at) VALUES (?, ?, ?, ?)",
        (source, headline, category, datetime.now())
    )
    conn.commit()
...
def scrape_site(page, name, url, selector=None):
    ...
    for title in unique_headlines[:10]:
        ...
        save_to_db(name, title, category)
        print(f"[{name}] {title} --> [{category}]")
